...
- `ALLOWED_ORIGINS` – comma-separated list of domains allowed to call Supabase functions (development convenience; production uses DB/live domains). `get_gateway_credentials` permits anonymous requests from these origins or from the store's own domains.
