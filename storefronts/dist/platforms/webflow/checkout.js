...
function ready4() {
  const number = document.querySelector('input[data-collect="cardNumber"]');
  const cvc = document.querySelector('input[data-collect="cvv"]');
...
async function getActivePaymentGateway(log5, warn5) {
  var _a5;
  const cfg = window.SMOOTHR_CONFIG || {};
  ...
  const storeId = cfg.storeId;
  if (!storeId) {
    warn5("Store ID missing; defaulting to stripe");
    return "stripe";
  }
  try {
    const { data, error } = await supabaseClient_default.from("store_settings").select("settings").eq("store_id", storeId).maybeSingle();
    if (error) {
      warn5("Store settings lookup failed:", error.message || error);
      return "stripe";
    }
    const gateway = (_a5 = data == null ? void 0 : data.settings) == null ? void 0 : _a5.active_payment_gateway;
    if (!gateway) {
      warn5("active_payment_gateway missing; defaulting to stripe");
      return "stripe";
    }
    return gateway;
  } catch (e) {
    warn5("Gateway lookup failed:", (e == null ? void 0 : e.message) || e);
    return "stripe";
  }
}
...
async function initCheckout() {
  ...
  if (!loader) {
    warn5(`Unknown payment gateway: ${provider}; falling back to stripe`);
  ...
  if (checkoutEl) {
    const attr = checkoutEl.hasAttribute("data-smoothr-pay") ? "[data-smoothr-pay]" : "[data-smoothr-checkout]";
    log5(`checkout trigger found ${attr}`, checkoutEl);
  } else {
    warn5("missing [data-smoothr-pay] or [data-smoothr-checkout]");
    return;
  }
  ...
  fields.forEach(([name, val]) => log5(`${name} = ${val}`));
  if (!emailField)
  ...
  checkoutEl == null ? void 0 : checkoutEl.addEventListener(eventName, async (event) => {
    ...
    const triggerAttr = checkoutEl.hasAttribute("data-smoothr-pay") ? "[data-smoothr-pay]" : "[data-smoothr-checkout]";
    log5(`${triggerAttr} triggered`);
...
document.addEventListener("DOMContentLoaded", initCheckout2);
export {
