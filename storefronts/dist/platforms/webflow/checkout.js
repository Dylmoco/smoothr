...
// storefronts/checkout/utils/forceStripeIframeStyle.js
      ...
      then(onfulfilled, onrejected) {
        ...
        let res = _fetch(this.url.toString(), {
          ...
          if (res2.ok) {
            ...
            const countHeader = (_a4 = this.headers["Prefer"]) === null || _a4 === void 0 ? void 0 : _a4.match(/count=(exact|planned|estimated)/);
...
var require_PostgrestTransformBuilder = __commonJS({
  "node_modules/@supabase/postgrest-js/dist/cjs/PostgrestTransformBuilder.js"(exports) {
    ...
    var PostgrestTransformBuilder2 = class extends PostgrestBuilder_1.default {
      ...
      rollback() {
        var _a4;
        if (((_a4 = this.headers["Prefer"]) !== null && _a4 !== void 0 ? _a4 : "").trim().length > 0) {
      ...
      subscribe(callback, timeout = this.timeout) {
        ...
        if (this.joinedOnce) {
          ...
          const config = {
            broadcast,
            presence,
            postgres_changes: (_b = (_a4 = this.bindings.postgres_changes) === null || _a4 === void 0 ? void 0 : _a4.map((r) => r.filter)) !== null && _b !== void 0 ? _b : [],
            private: isPrivate
          };
    ...
    RealtimeClient = class {
      ...
      async sendHeartbeat() {
        ...
        if (this.pendingHeartbeatRef) {
          ...
          (_a4 = this.conn) === null || _a4 === void 0 ? void 0 : _a4.close(WS_CLOSE_NORMAL, "hearbeat timeout");
          return;
        }
...
async function handleError2(error) {
  ...
  if (!errorCode) {
    ...
    throw new AuthWeakPasswordError(_getErrorMessage2(data), error.status, ((_a4 = data.weak_password) === null || _a4 === void 0 ? void 0 : _a4.reasons) || []);
  ...
  "node_modules/@supabase/auth-js/dist/module/GoTrueClient.js"() {
    ...
    GoTrueClient = class _GoTrueClient {
      ...
      constructor(options) {
        ...
        } else if (isBrowser() && ((_a4 = globalThis === null || globalThis === void 0 ? void 0 : globalThis.navigator) === null || _a4 === void 0 ? void 0 : _a4.locks)) {
      ...
      async _recoverAndRefresh() {
        ...
        try {
          ...
          const expiresWithMargin = ((_a4 = currentSession.expires_at) !== null && _a4 !== void 0 ? _a4 : Infinity) * 1e3 - Date.now() < EXPIRY_MARGIN_MS;
    ...
    SupabaseClient = class {
      ...
      _getAccessToken() {
        ...
        return __awaiter8(this, void 0, void 0, function* () {
          ...
          return (_b = (_a4 = data.session) === null || _a4 === void 0 ? void 0 : _a4.access_token) !== null && _b !== void 0 ? _b : null;
        });
      }
      _initSupabaseAuthClient({ autoRefreshToken, persistSession, detectSessionInUrl, storage, storageKey, flowType, lock, debug: debug2 }, headers, fetch3) {
        const authHeaders = {
...
async function getPublicCredential(storeId, integrationId, gateway) {
  ...
  try {
    if (integrationId === "nmi" || gateway === "nmi") {
      const { data: data2, error: error2 } = await supabase.from("public_store_integration_credentials").select("tokenization_key").eq("store_id", storeId).eq("gateway", gateway || integrationId).maybeSingle();
      if (error2) {
        console.warn("[Smoothr] Credential lookup failed:", error2.message || error2);
        return null;
      }
      return data2 ? { tokenization_key: data2.tokenization_key } : null;
    }
    if (integrationId === "stripe" || gateway === "stripe") {
      const { data: data2, error: error2 } = await supabase.from("public_store_integration_credentials").select("publishable_key").eq("store_id", storeId).eq("gateway", "stripe").maybeSingle();
      if (error2) {
        console.warn("[Smoothr] Credential lookup failed:", error2.message || error2);
        return null;
      }
      if (data2 == null ? void 0 : data2.publishable_key) {
        console.log("[Smoothr] Loaded Stripe key from Supabase.");
        return { publishable_key: data2.publishable_key };
      }
      console.warn("[Smoothr] Stripe publishable key not found");
      return null;
    }
    let query = supabase.from("store_integrations").select("api_key, settings").eq("store_id", storeId);
    if (gateway) {
      query = query.or(
...
function handleSuccessRedirect(res, data) {
  ...
  if (res.ok && data.success) {
    (_b = (_a4 = Smoothr == null ? void 0 : Smoothr.cart) == null ? void 0 : _a4.clearCart) == null ? void 0 : _b.call(_a4);
    window.location.href = "/checkout-success";
  }
}
var init_handleSuccessRedirect = __esm({
  "storefronts/checkout/utils/handleSuccessRedirect.js"() {
  }
});

// storefronts/core/payments/stripeStyle.js
function buildStripeElementStyle(el) {
...
async function getStoreSettings(storeId) {
  if (!storeId)
    return null;
  try {
    const { data, error } = await supabase.from("public_store_settings").select("*").eq("store_id", storeId).maybeSingle();
    if (error) {
...
var init_stripe = __esm({
  "storefronts/checkout/gateways/stripe.js"() {
    ...
    fieldsMounted = false;
    mountAttempts = 0;
    debug = (_a = window.SMOOTHR_CONFIG) == null ? void 0 : _a.debug;
    log = (...args) => debug && console.log("[Smoothr Stripe]", ...args);
    warn = (...args) => debug && console.warn("[Smoothr Stripe]", ...args);
...
// storefronts/checkout/utils/cartHash.js
async function computeCartHash(cart, total, email) {
  const normalized = [...cart].map((item) => ({ id: item.product_id, qty: item.quantity })).sort((a, b) => a.id > b.id ? 1 : a.id < b.id ? -1 : 0);
...
async function createPaymentMethod2() {
  ...
  const { acceptReady: acceptReady2, authorizeNetReady: authorizeNetReady2, isSubmitting: isSubmitting3 } = getReadinessState();
  log2("createPaymentMethod readiness", {
    acceptReady: acceptReady2,
    authorizeNetReady: authorizeNetReady2,
    isSubmitting: isSubmitting3
  });
  if (!acceptReady2) {
  ...
  return new Promise((resolve) => {
    ...
    try {
      window.Accept.dispatchData(secureData, (response) => {
        var _a5, _b2, _c2, _d2, _e2, _f2, _g, _h;
        clearTimeout(timeoutId);
        console.log("[AuthorizeNet] Full dispatchData response:", response);
        log2("\u{1F501} dispatchData response:", response);
        if (((_a5 = response.messages) == null ? void 0 : _a5.resultCode) === "Ok" && ((_b2 = response.opaqueData) == null ? void 0 : _b2.dataDescriptor) && ((_c2 = response.opaqueData) == null ? void 0 : _c2.dataValue)) {
          submitting = false;
          updateDebug();
...
async function mountCardFields3() {
  ...
  const paypalButtons = window.paypal.Buttons({
    createOrder: async () => {
      ...
      const total = ((_c2 = (_b2 = (_a5 = window.Smoothr) == null ? void 0 : _a5.cart) == null ? void 0 : _b2.getTotal) == null ? void 0 : _c2.call(_b2)) || parseInt(((_d2 = totalEl == null ? void 0 : totalEl.textContent) == null ? void 0 : _d2.replace(/[^0-9]/g, "")) || "0", 10) || 0;
    ...
    onApprove: async (data) => {
      var _a5, _b2, _c2, _d2, _e, _f, _g, _h, _i, _j, _k, _l, _m, _n, _o, _p, _q, _r, _s, _t, _u, _v, _w, _x, _y, _z, _A, _B, _C, _D, _E, _F, _G, _H, _I, _J, _K, _L, _M, _N, _O, _P, _Q, _R, _S, _T;
      const q = (s) => document.querySelector(s);
      const first = ((_b2 = (_a5 = q("[data-smoothr-first-name]")) == null ? void 0 : _a5.value) == null ? void 0 : _b2.trim()) || "";
...
var mounted, isSubmitting, paypal_default;
...
async function resolveTokenizationKey() {
  ...
  try {
    ...
    if (cred == null ? void 0 : cred.api_key) {
      cachedKey2 = cred.api_key;
    } else if (cred == null ? void 0 : cred.tokenization_key) {
      cachedKey2 = cred.tokenization_key;
    } else if ((_c = cred == null ? void 0 : cred.settings) == null ? void 0 : _c.tokenization_key) {
      warn3("tokenization_key is deprecated \u2013 update integration to use api_key");
      cachedKey2 = cred.settings.tokenization_key;
    } else {
      cachedKey2 = null;
    }
  } catch (e) {
  ...
  try {
    ...
    CollectJS.configure({
      ...
      callback(response) {
        ...
        const buttons2 = Array.from(document.querySelectorAll("[data-smoothr-pay]"));
        if (!response.token) {
          console.error("[NMI] Tokenization failed", response.reason);
          alert("Please check your payment details and try again.");
          resetSubmission(buttons2);
          return;
        }
        console.log("[NMI] Token:", response.token);
        const firstName = ((_a4 = document.querySelector("[data-smoothr-first-name]")) == null ? void 0 : _a4.value) || "";
        ...
        let billLine1 = sameBilling ? shipLine1 : ((_j = document.querySelector("[data-smoothr-bill-line1]")) == null ? void 0 : _j.value) || "";
        let billLine2 = sameBilling ? shipLine2 : ((_k = document.querySelector("[data-smoothr-bill-line2]")) == null ? void 0 : _k.value) || "";
        let billCity = sameBilling ? shipCity : ((_l = document.querySelector("[data-smoothr-bill-city]")) == null ? void 0 : _l.value) || "";
        let billState = sameBilling ? shipState : ((_m = document.querySelector("[data-smoothr-bill-state]")) == null ? void 0 : _m.value) || "";
        let billPostal = sameBilling ? shipPostal : ((_n = document.querySelector("[data-smoothr-bill-postal]")) == null ? void 0 : _n.value) || "";
        let billCountry = sameBilling ? shipCountry : ((_o = document.querySelector("[data-smoothr-bill-country]")) == null ? void 0 : _o.value) || "";
...
var hasMounted, isConfigured, isLocked, isSubmitting2, configPromise, resolveConfig, mountNMI, nmi_default;
var init_nmi2 = __esm({
  "storefronts/checkout/gateways/nmi.js"() {
    init_nmi();
    init_handleSuccessRedirect();
    ...
    if (typeof window !== "undefined") {
      ...
      if (document.readyState === "complete")
        ...
        document.addEventListener("DOMContentLoaded", mountCardFields4);
    }
  }
});
...
var fieldsMounted3, segpay_default;
...
// storefronts/checkout/utils/checkoutLogger.js
function checkoutLogger(block = document) {
  var _a4;
  const debug2 = (_a4 = window.SMOOTHR_CONFIG) == null ? void 0 : _a4.debug;
  const log4 = (...args) => debug2 && console.log("[Smoothr Checkout]", ...args);
  const warn4 = (...args) => console.warn("[Smoothr Checkout]", ...args);
  const err = (...args) => debug2 && console.error("[Smoothr Checkout]", ...args);
  const select = (sel) => {
    if (typeof process !== "undefined" && false) {
  ...
  return { log: log4, warn: warn4, err, select, q };
}

// core/utils/resolveGateway.js
var SUPPORTED_GATEWAYS = ["stripe", "authorizeNet", "paypal", "nmi", "segpay"];
function resolveGateway(config = {}, storeSettings = {}) {
  const provider = config.active_payment_gateway || storeSettings.active_payment_gateway;
  if (!provider) {
...
// storefronts/checkout/utils/resolveGateway.js
init_stripe();
async function getActivePaymentGateway(log4 = () => {
}, warn4 = () => {
}) {
  ...
  try {
    ...
    warn4("Gateway resolution failed:", (e == null ? void 0 : e.message) || e);
    throw e;
  }
}

// storefronts/checkout/utils/collectFormFields.js
function collectFormFields(q) {
  return {
...
// storefronts/checkout/utils/constructPayload.js
function constructPayload(provider, token, info) {
  const {
    email,
    first_name,
    last_name,
  ...
  if (provider === "stripe") {
    ...
    payload.payment_method = token == null ? void 0 : token.id;
  }
  return payload;
}

// storefronts/checkout/utils/gatewayDispatcher.js
init_handleSuccessRedirect();
async function gatewayDispatcher(provider, payload, token, log4, warn4, err) {
  var _a4;
  const apiBase = ((_a4 = window.SMOOTHR_CONFIG) == null ? void 0 : _a4.apiBase) || "";
  log4("POST", `${apiBase}/api/checkout/${provider}`);
  ...
  try {
    if (provider === "authorizeNet") {
      ...
      log4("create-order response", orderRes.status, orderData);
  ...
  log4("fetch response", res.status, data);
...
var gatewayLoaders = {
  ...
  nmi: () => Promise.resolve().then(() => (init_nmi2(), nmi_exports)),
...
async function initCheckout(config) {
  ...
  const { log: log4, warn: warn4, err, select, q } = checkoutLogger();
  log4("SDK initialized");
  log4("SMOOTHR_CONFIG", JSON.stringify(window.SMOOTHR_CONFIG));
  const provider = await getActivePaymentGateway(log4, warn4);
  const loader = gatewayLoaders[provider];
  if (!loader)
    throw new Error(`Unknown payment gateway: ${provider}`);
  const gateway = (await loader()).default;
  log4(`Using gateway: ${provider}`);
  ...
  if (!checkoutEl) {
    warn4("Missing [data-smoothr-pay]");
    return;
  }
  log4("checkout trigger found", checkoutEl);
  const fields = collectFormFields(q);
  let { emailField } = fields;
  if (!emailField)
    emailField = await select("[data-smoothr-email]");
  ...
  while (attempts < 2 && !gateway.isMounted()) {
    try {
      ...
      warn4("Mount failed:", e.message);
    }
    attempts++;
  }
  if (!gateway.isMounted()) {
    warn4("Gateway mount failed");
    return;
  }
  bindCardInputs();
  if (provider === "nmi") {
  ...
  payButtons.forEach((btn) => {
    btn.addEventListener(eventName, async (e) => {
      ...
      e.stopPropagation();
      if (isSubmitting3) {
        warn4("Checkout already in progress");
        return;
      }
      isSubmitting3 = true;
      forEachPayButton(disableButton);
      clearErrorMessages();
      log4("[data-smoothr-pay] triggered");
      const {
        email,
        first_name,
        last_name,
      ...
      const total = Math.round(
        (((_b = (_a4 = window.Smoothr.cart).getTotal) == null ? void 0 : _b.call(_a4)) || parseFloat(totalEl.textContent.replace(/[^0-9.]/g, "")) || 0) * 100
      );
      const currency = window.SMOOTHR_CONFIG.baseCurrency;
      const customer_id = ((_c = window.smoothr.auth.user) == null ? void 0 : _c.id) || null;
      ...
      if (last.hash === cartHash && last.success && Date.now() - last.timestamp < 6e4) {
  ...
  log4(`${eventName} handler attached`);
}
function collectFormData(fields, emailField) {
  var _a4, _b, _c, _d, _e, _f, _g, _h, _i, _j, _k, _l, _m, _n, _o, _p, _q;
  const email = (emailField == null ? void 0 : emailField.value.trim()) || "";
  const first_name = ((_a4 = fields.firstName) == null ? void 0 : _a4.value.trim()) || "";
  const last_name = ((_b = fields.lastName) == null ? void 0 : _b.value.trim()) || "";
  const ship = {
    line1: ((_c = fields.ship_line1) == null ? void 0 : _c.value.trim()) || "",
    line2: ((_d = fields.ship_line2) == null ? void 0 : _d.value.trim()) || "",
    city: ((_e = fields.ship_city) == null ? void 0 : _e.value.trim()) || "",
    state: ((_f = fields.ship_state) == null ? void 0 : _f.value.trim()) || "",
    postal_code: ((_g = fields.ship_postal) == null ? void 0 : _g.value.trim()) || "",
    country: ((_h = fields.ship_country) == null ? void 0 : _h.value.trim()) || ""
  };
  const same = (_i = document.querySelector("[data-smoothr-billing-same-as-shipping]")) == null ? void 0 : _i.checked;
  ...
  if (same) {
    ...
    bill_first_name = ((_j = fields.bill_first_name) == null ? void 0 : _j.value.trim()) || "";
    bill_last_name = ((_k = fields.bill_last_name) == null ? void 0 : _k.value.trim()) || "";
    billing = {
      name: `${bill_first_name} ${bill_last_name}`.trim(),
      address: {
        line1: ((_l = fields.bill_line1) == null ? void 0 : _l.value.trim()) || "",
        line2: ((_m = fields.bill_line2) == null ? void 0 : _m.value.trim()) || "",
        city: ((_n = fields.bill_city) == null ? void 0 : _n.value.trim()) || "",
        state: ((_o = fields.bill_state) == null ? void 0 : _o.value.trim()) || "",
        postal_code: ((_p = fields.bill_postal) == null ? void 0 : _p.value.trim()) || "",
        country: ((_q = fields.bill_country) == null ? void 0 : _q.value.trim()) || ""
      }
    };
  }
...
function validateFormData({ email, first_name, last_name, shipping }) {
  const errs = [];
  if (!email)
    errs.push({ field: "email", message: "Email required" });
  if (!first_name)
    errs.push({ field: "first_name", message: "First name required" });
  if (!last_name)
    errs.push({ field: "last_name", message: "Last name required" });
  const addr = shipping.address;
  if (!addr.line1)
    errs.push({ field: "ship_line1", message: "Street required" });
  if (!addr.city)
    errs.push({ field: "ship_city", message: "City required" });
  if (!addr.state)
    errs.push({ field: "ship_state", message: "State required" });
  if (!addr.postal_code)
    errs.push({ field: "ship_postal", message: "Postal required" });
  if (!addr.country)
    errs.push({ field: "ship_country", message: "Country required" });
  return errs;
}
...
function showUserMessage(text, type = "info") {
  ...
  if (type === "success")
    setTimeout(hideUserMessage, 5e3);
}
...
function getPaymentMethodErrorMessage(err) {
  ...
  if (m.includes("expiry"))
    return "Check your card expiry.";
  if (m.includes("cvv"))
    return "Check your security code.";
  return "Please check payment details.";
}
function handleCheckoutError(res, data, hash) {
  if (res.status === 409)
    showUserMessage("Order already submitted.", "warning");
  else
    showUserMessage(data.error || "Checkout failed", "error");
...
(function addNmiPerformanceHints() {
  ...
  head2.appendChild(dnsPrefetch);
  const preconnect = document.createElement("link");
  preconnect.rel = "preconnect";
  preconnect.href = origin;
  preconnect.crossOrigin = "anonymous";
  head2.appendChild(preconnect);
  const preload = document.createElement("link");
  preload.rel = "preload";
  preload.href = script;
  preload.as = "script";
  head2.appendChild(preload);
  try {
