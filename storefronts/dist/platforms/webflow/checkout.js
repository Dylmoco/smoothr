...
var _getErrorMessage = (err2) => err2.msg || err2.message || err2.error_description || err2.error || JSON.stringify(err2);
var handleError = (error, reject, options) => __awaiter3(void 0, void 0, void 0, function* () {
  const Res = yield resolveResponse();
  if (error instanceof Res && !(options === null || options === void 0 ? void 0 : options.noResolveJson)) {
    error.json().then((err2) => {
      reject(new StorageApiError(_getErrorMessage(err2), error.status || 500));
    }).catch((err2) => {
      reject(new StorageUnknownError(_getErrorMessage(err2), err2));
    });
  } else {
...
async function resolveTokenizationKey() {
  ...
  try {
    ...
    cachedKey = ((_c = cred == null ? void 0 : cred.settings) == null ? void 0 : _c.tokenization_key) || null;
  } catch (e) {
...
async function validateDiscount(code) {
  ...
  try {
    ...
    if (data.expires_at && new Date(data.expires_at).getTime() < now)
      return null;
    if (typeof data.max_redemptions === "number" && typeof data.redemptions === "number" && data.max_redemptions > 0 && data.redemptions >= data.max_redemptions)
      return null;
...
function writeCart(cart) {
  ...
  if (!storage)
    return;
  try {
    storage.setItem(STORAGE_KEY2, JSON.stringify(cart));
  } catch (err2) {
    err2("write failed", err2);
  }
}
...
function showError(form, msg, input, trigger) {
  const target = findMessageContainer(trigger || form, "[data-smoothr-error]") || form.querySelector("[data-smoothr-error]");
  if (target) {
...
function showSuccess(form, msg, trigger) {
  const target = findMessageContainer(trigger || form, "[data-smoothr-success]") || form.querySelector("[data-smoothr-success]");
  if (target) {
    target.removeAttribute("hidden");
    target.textContent = msg;
    target.style.display = "";
    ...
    log3("No [data-smoothr-success] container found");
    alert(msg);
  }
}
