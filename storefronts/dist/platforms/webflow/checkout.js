...
function checkoutLogger(block = document) {
  var _a5;
  const debug2 = (_a5 = window.SMOOTHR_CONFIG) == null ? void 0 : _a5.debug;
  const log5 = (...args) => debug2 && console.log("[Smoothr Checkout]", ...args);
  const warn5 = (...args) => console.warn("[Smoothr Checkout]", ...args);
  ...
  return { log: log5, warn: warn5, err, select, q };
}
var init_checkoutLogger = __esm({
  "storefronts/checkout/utils/checkoutLogger.js"() {
    init_waitForElement();
  }
});
...
function resolveGateway(config = {}, storeSettings = {}) {
  var _a5;
  const provider = config.active_payment_gateway || storeSettings.active_payment_gateway || ((_a5 = storeSettings.settings) == null ? void 0 : _a5.active_payment_gateway);
  if (!provider) {
...
var init_resolveGateway = __esm({
  "core/utils/resolveGateway.js"() {
    SUPPORTED_GATEWAYS = ["stripe", "authorizeNet", "paypal", "nmi", "segpay"];
  }
});
...
async function resolveStripeKey() {
  ...
  if (key) {
    ...
    if (storeId) {
      ...
      if (!key) {
        try {
          const { data, error } = await supabaseClient_default.from("store_integrations").select("api_key, settings").eq("store_id", storeId).eq("provider", "stripe").maybeSingle();
          if (error) {
            warn("Integration lookup failed:", error.message || error);
          } else if (data) {
            key = data.api_key || ((_a5 = data.settings) == null ? void 0 : _a5.public_key) || "";
            if (key) {
              log(
  ...
  mountPromise = (async () => {
    ...
    if (numberTarget && !existingNumber) {
      ...
      setTimeout(() => {
        ...
        if (iframe && width < 10) {
          ...
          (_a5 = cardNumberElement == null ? void 0 : cardNumberElement.unmount) == null ? void 0 : _a5.call(cardNumberElement);
    ...
    if (expiryTarget && !existingExpiry) {
      ...
      setTimeout(() => {
        ...
        if (iframe && width < 10) {
          console.warn("[Stripe] iframe dead \u2192 remounting now...");
          (_a5 = el == null ? void 0 : el.unmount) == null ? void 0 : _a5.call(el);
...
async function getActivePaymentGateway(log5 = () => {
}, warn5 = () => {
}) {
  const cfg = window.SMOOTHR_CONFIG || {};
  ...
  try {
    ...
    warn5("Gateway resolution failed:", (e == null ? void 0 : e.message) || e);
    throw e;
  }
}
var init_resolveGateway2 = __esm({
  "storefronts/checkout/utils/resolveGateway.js"() {
    init_resolveGateway();
    init_stripe();
  }
...
var init_collectFormFields = __esm({
  "storefronts/checkout/utils/collectFormFields.js"() {
  }
});
...
var init_constructPayload = __esm({
  "storefronts/checkout/utils/constructPayload.js"() {
  }
});

// storefronts/checkout/utils/gatewayDispatcher.js
async function gatewayDispatcher(provider, payload, token, log5, warn5, err) {
  var _a5;
  const apiBase = ((_a5 = window.SMOOTHR_CONFIG) == null ? void 0 : _a5.apiBase) || "";
  log5("POST", `${apiBase}/api/checkout/${provider}`);
  ...
  log5("fetch response", res.status, data);
...
var init_gatewayDispatcher = __esm({
  "storefronts/checkout/utils/gatewayDispatcher.js"() {
    init_handleSuccessRedirect();
  }
});
...
async function createPaymentMethod4() {
  ...
  const expiryEl = ((_a5 = document.querySelector("[data-smoothr-card-expiry]")) == null ? void 0 : _a5.querySelector("input[data-smoothr-expiry-visible]")) || ((_b = document.querySelector("[data-smoothr-card-expiry]")) == null ? void 0 : _b.querySelector("input")) || document.querySelector("[data-smoothr-card-expiry]");
  const expiryRaw = (expiryEl == null ? void 0 : expiryEl.value) || "";
  const match = expiryRaw.replace(/\s+/g, "").match(/^(\d{1,2})\/?(\d{2,4})$/);
  if (!match) {
    warn4("Invalid expiry format:", expiryRaw);
  ...
  const expYear = year;
  log4("Parsed expiry", { expMonth, expYear });
  return new Promise((resolve) => {
    try {
...
var init_nmi2 = __esm({
  "storefronts/checkout/gateways/nmi.js"() {
    init_nmi();
    init_waitForElement();
    DEBUG3 = !!((_a4 = window.SMOOTHR_CONFIG) == null ? void 0 : _a4.debug);
    log4 = (...a) => DEBUG3 && console.log("[NMI]", ...a);
    warn4 = (...a) => DEBUG3 && console.warn("[NMI]", ...a);
    nmi_default = {
      mountCardFields: mountNMI,
      isMounted: isMounted4,
      ready: ready4,
...
async function initCheckout(config) {
  ...
  let { log: log5, warn: warn5, err, select, q } = checkoutLogger();
  log5("SDK initialized");
  log5("SMOOTHR_CONFIG", JSON.stringify(window.SMOOTHR_CONFIG));
  const provider = await getActivePaymentGateway(log5, warn5);
  const loader = gatewayLoaders[provider];
  if (!loader) {
    throw new Error(`Unknown payment gateway: ${provider}`);
  }
  const gateway = (await loader()).default;
  log5(`Using gateway: ${provider}`);
  ...
  if (provider === "stripe") {
    let stripeKey = (_a5 = window.SMOOTHR_CONFIG) == null ? void 0 : _a5.stripeKey;
    if (!stripeKey) {
      const storeId = (_b = window.SMOOTHR_CONFIG) == null ? void 0 : _b.storeId;
      const cred = await getPublicCredential(storeId, "stripe");
      stripeKey = (cred == null ? void 0 : cred.api_key) || ((_c = cred == null ? void 0 : cred.settings) == null ? void 0 : _c.publishable_key) || "";
      if (stripeKey)
        window.SMOOTHR_CONFIG.stripeKey = stripeKey;
    }
    log5(`stripeKey: ${stripeKey}`);
    if (!stripeKey) {
      warn5("\u274C Failed at Stripe Key Check: missing key");
      console.log("[Smoothr Checkout] No Stripe key provided");
      return;
    }
    log5("Stripe key confirmed");
  }
  ...
  if (checkoutEl) {
    ...
    warn5("missing [data-smoothr-pay]");
    return;
  }
  const block = ((_d = checkoutEl.closest) == null ? void 0 : _d.call(checkoutEl, "[data-smoothr-product-id]")) || document;
  const productId = ((_e = checkoutEl.dataset) == null ? void 0 : _e.smoothrProductId) || ((_f = block.dataset) == null ? void 0 : _f.smoothrProductId);
  ...
  logFields.forEach(([name, val]) => log5(`${name} = ${val}`));
  if (!emailField)
    warn5("missing [data-smoothr-email]");
  if (!totalEl)
    warn5("missing [data-smoothr-total]");
  log5("no polling loops active");
  let mountAttempts2 = 0;
  const maxAttempts = 1;
  while (mountAttempts2 < maxAttempts && !gateway.isMounted()) {
    log5(`Attempting to mount gateway, attempt ${mountAttempts2 + 1}`);
    try {
      await gateway.mountCardFields();
    } catch (e) {
      warn5("Mount attempt failed:", e.message);
    }
    mountAttempts2++;
  }
  if (!gateway.isMounted()) {
    warn5("Gateway failed to mount after retries");
    return;
  }
  bindCardInputs();
  const isForm = ((_g = checkoutEl.tagName) == null ? void 0 : _g.toLowerCase()) === "form";
  const eventName = isForm ? "submit" : "click";
  payButtons.forEach((btn) => {
    btn.addEventListener(eventName, async (event) => {
      var _a6, _b2, _c2, _d2, _e2, _f2, _g2, _h, _i, _j, _k, _l, _m, _n, _o, _p, _q, _r, _s, _t, _u, _v, _w, _x, _y, _z, _A, _B, _C, _D, _E, _F, _G, _H, _I, _J, _K, _L, _M, _N, _O, _P, _Q, _R;
      event.preventDefault();
      event.stopPropagation();
      if (isSubmitting) {
        warn5("Checkout already in progress");
        return;
      }
      isSubmitting = true;
      if ("disabled" in btn)
        btn.disabled = true;
      log5("[data-smoothr-pay] triggered");
      ...
      const first_name = ((_d2 = (_c2 = fields.firstName) == null ? void 0 : _c2.value) == null ? void 0 : _d2.trim()) || "";
      const last_name = ((_f2 = (_e2 = fields.lastName) == null ? void 0 : _e2.value) == null ? void 0 : _f2.trim()) || "";
      const line1 = ((_h = (_g2 = fields.ship_line1) == null ? void 0 : _g2.value) == null ? void 0 : _h.trim()) || "";
      const line2 = ((_j = (_i = fields.ship_line2) == null ? void 0 : _i.value) == null ? void 0 : _j.trim()) || "";
      const city = ((_l = (_k = fields.ship_city) == null ? void 0 : _k.value) == null ? void 0 : _l.trim()) || "";
      const state = ((_n = (_m = fields.ship_state) == null ? void 0 : _m.value) == null ? void 0 : _n.trim()) || "";
      const postal_code = ((_p = (_o = fields.ship_postal) == null ? void 0 : _o.value) == null ? void 0 : _p.trim()) || "";
      const country = ((_r = (_q = fields.ship_country) == null ? void 0 : _q.value) == null ? void 0 : _r.trim()) || "";
      const shipping = {
        name: `${first_name} ${last_name}`,
        address: { line1, line2, city, state, postal_code, country }
      };
      const bill_first_name = ((_t = (_s = fields.bill_first_name) == null ? void 0 : _s.value) == null ? void 0 : _t.trim()) || "";
      const bill_last_name = ((_v = (_u = fields.bill_last_name) == null ? void 0 : _u.value) == null ? void 0 : _v.trim()) || "";
      const bill_line1 = ((_x = (_w = fields.bill_line1) == null ? void 0 : _w.value) == null ? void 0 : _x.trim()) || "";
      const bill_line2 = ((_z = (_y = fields.bill_line2) == null ? void 0 : _y.value) == null ? void 0 : _z.trim()) || "";
      const bill_city = ((_B = (_A = fields.bill_city) == null ? void 0 : _A.value) == null ? void 0 : _B.trim()) || "";
      const bill_state = ((_D = (_C = fields.bill_state) == null ? void 0 : _C.value) == null ? void 0 : _D.trim()) || "";
      const bill_postal = ((_F = (_E = fields.bill_postal) == null ? void 0 : _E.value) == null ? void 0 : _F.trim()) || "";
      const bill_country = ((_H = (_G = fields.bill_country) == null ? void 0 : _G.value) == null ? void 0 : _H.trim()) || "";
      const billing = {
      ...
      const Smoothr2 = window.Smoothr || window.smoothr;
      const cart = ((_I = Smoothr2 == null ? void 0 : Smoothr2.cart) == null ? void 0 : _I.getCart()) || { items: [] };
      const total = ((_K = (_J = Smoothr2 == null ? void 0 : Smoothr2.cart) == null ? void 0 : _J.getTotal) == null ? void 0 : _K.call(_J)) || parseInt(((totalEl == null ? void 0 : totalEl.textContent) || "0").replace(/[^0-9]/g, ""), 10) || 0;
      const currency = ((_L = window.SMOOTHR_CONFIG) == null ? void 0 : _L.baseCurrency) || "USD";
      const customer_id = ((_O = (_N = (_M = window.smoothr) == null ? void 0 : _M.auth) == null ? void 0 : _N.user) == null ? void 0 : _O.id) || null;
      const store_id = (_P = window.SMOOTHR_CONFIG) == null ? void 0 : _P.storeId;
      ...
      try {
        ...
        if ((_R = window.SMOOTHR_CONFIG) == null ? void 0 : _R.debug) {
...
var init_checkout = __esm({
  "storefronts/checkout/checkout.js"() {
    ...
    init_checkoutLogger();
    init_resolveGateway2();
    init_collectFormFields();
    init_constructPayload();
    init_gatewayDispatcher();
    (() => {
