...
/**
 * Force Stripe iframe to fill its container precisely.
 */
  ...
  const interval = setInterval(()=>{
    ...
    if (iframe && container) {
      Object.assign(iframe.style, {
        position:'absolute', top:'0', left:'0', width:'100%', height:`${container.offsetHeight}px`,
...
function getStripeFieldCss(selector, placeholderSelector) {
  ...
  return {
    style:{
      base:{
        backgroundColor:'transparent', color:fieldStyle.color, fontFamily:fieldStyle.fontFamily,
        fontSize:fieldStyle.fontSize, fontStyle:fieldStyle.fontStyle, fontWeight:fieldStyle.fontWeight,
        letterSpacing:fieldStyle.letterSpacing, textAlign:fieldStyle.textAlign, textShadow:fieldStyle.textShadow,
        '::placeholder':{
          color:placeholderColor, fontFamily:placeholderStyle.fontFamily,
          fontSize:placeholderStyle.fontSize, fontStyle:placeholderStyle.fontStyle,
          fontWeight:placeholderStyle.fontWeight, letterSpacing:placeholderStyle.letterSpacing,
...
export async function getElements(){
  ...
  const key=await resolveStripeKey(); if(!key) return {stripe:null,elements:null};
  stripe=Stripe(key); elements=stripe.elements(); return {stripe,elements};
}

export async function mountCardFields(){
  if(mountPromise) return mountPromise; if(fieldsMounted) return;
  mountPromise=(async()=>{ const {elements:els}=await getElements(); if(!els) return; fieldsMounted=true;
    const mounts=[
    ...
    for(const {type,selector,placeholder} of mounts){
      ...
      const el=els.create(type,{style,placeholder:placeholderText}); el.mount(selector);
      forceStripeIframeStyle(selector);
      if(type==='cardNumber') cardNumberElement=el;
      if(type==='cardExpiry') cardExpiryElement=el;
      if(type==='cardCvc') cardCvcElement=el;
    }
  })();
  mountPromise=mountPromise.finally(()=>{mountPromise=null;}); return mountPromise;
}
...
export async function getStoreSettings(storeId){
  if(!storeId) return null; try{ const {data,error}=await supabase.from('public_store_settings').select('*').eq('store_id',storeId).maybeSingle(); if(error) return null; return data;}catch{return null;}
}

export async function createPaymentMethod(billing_details){
  if(!ready()) return {error:{message:'Stripe not ready'}};
  const {stripe:inst}=await getElements(); if(!inst) return {error:{message:'Stripe not ready'}};
